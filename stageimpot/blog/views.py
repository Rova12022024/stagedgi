        ...
        if logForm.is_valid():
            ...
            if user is not None:
                ...
                if user.is_superuser == True:
                    ...
                    id_stage = user.id
                    usercomplete = userCompleteModel.objects.filter(user_id__exact=id_stage).values()
                    if usercomplete:                      
...
def detailStage(request, id):
    ...
    id_complete = userCompleteModel.objects.get(user_id__exact=users_id)    
...
def completeStage(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'userComplete.html',{'completeUserForm':userForm})
